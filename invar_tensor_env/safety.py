...
class Safety:
        ...
        if not further_link:
            ...
            for i in range(len(shape)):
                if i+1 < len(shape):
                    ...
                    if euklid_dist_to_intercept < 2.5:
    ...
    def getTrafficSafetyMeasures(self):
        ...
        for i in range(len(self.distances_tfc)):
            ...
            #dist_to_poc_triv = self.getDistance(traci.lane.getShape(self.distances[i][-4])[1], self.distances[i][1])
            tfc_tbi_tai = []
            for j in range(len(self.distances_tfc[i][2])):
                ...
                if traci.vehicle.getLaneID(self.distances_tfc[i][2][j][0]) == self.distances[i][-4]:
                    ...
                    #print(self.distances_tfc[i][0], traci.vehicle.getPosition(self.distances_tfc[i][2][j][0]))
    ...
    def getPriotizedTraffic(self, ego_safety, trc_safety):
        ...
        #print(priority_measures_per_link)
